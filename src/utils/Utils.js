...
function calculatePositiveDiagonal({ x, y }, range, filled=true) {
  ...
  if(!filled) {
    res = res.filter(({x: dx, y: dy}) => (dx === x - range && dy === y + range) || (dx === x + range && dy === y - range));
  }
...
function calculateRadius({x, y}, range, filled=true) {
  ...
  if(!filled) {
    ...
    verticalArray = verticalArray.filter(({_, y: dy}) => dy === y + range || dy === y - range);
  }
  else {
    for(let i = 0; i < verticalArray.length; i++) {
      horizontalArray.push(...calculateHorizontal({x, y: verticalArray[i].y}, Math.abs(Math.abs(verticalArray[i].y - y)-range)));
    }
  }
